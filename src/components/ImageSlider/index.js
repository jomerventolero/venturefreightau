...
const ImageSlider = () => {
  ...
  return (
    <div className='max-w-[1800px] h-[580px] sm:w-3/4 w-full transition-all duration-300 m-auto p-0 sm:py-16 relative group place-items-center z-10 flex flex-col'>
      ...
      <motion.div 
          ...
          className="flex flex-col content-evenly cursor-pointer absolute z-50 top-80 sm:top-18 text-center pt-10 sm:px-40 text-white drop-shadow-2xl text-xl font-bold">{slides[currentIndex].header}
          <br/><br/>
          <a className={`p-2 w-1/4 bg-blue-400 rounded-sm hover:bg-blue-600 mx-auto text-xs sm:text-lg shadow drop-shadow-xl ${slides[currentIndex].header === "" ? "hidden" : "block"}`} href={slides[currentIndex].location}>Learn More</a>
      </motion.div>
